<template>
	<view class="uni-easyinput" :class="{'uni-easyinput-error':msg}"
		:style="{color:inputBorder && msg?'#FF5454':styles.color}">
		<view class="uni-easyinput__content"
			...
			:style="{'border-color':inputBorder && msg?'#FF5454':styles.borderColor,'background-color':disabled?styles.disableColor:''}">
			...
			<input v-else :type="type === 'password'?'text':type" class="uni-easyinput__content-input" :style="{
				 'padding-right':type === 'password' ||clearable || prefixIcon?'30rpx':'10px',
				 'padding-left':prefixIcon?'':'30rpx'
			...
			<template v-if="type === 'password' && passwordIcon">
				<!-- <uni-icons v-if="val != '' " class="content-clear-icon" :class="{'is-textarea-icon':type==='textarea'}"
					:type="showPassword?'eye-filled':'eye-slash-filled'" :size="18" color="#c0c4cc" @click="onEyes">
				</uni-icons> -->
				<image v-if="val != '' " :src="`/static/icon/eye${showPassword?'':'-close'}@2x.png`" mode="widthFix"
					class="eye-icon" @click="onEyes" />
...
<style lang="scss">
	...
	.is-input-error-border {
		...
		.uni-easyinput__placeholder-class {
			color: mix(#fff, $uni-color-error, 50%);
			;
		}
	}
